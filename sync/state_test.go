...
func TestState_ApplyFullUpdate(t *testing.T) {
	...
	if torrent, ok := torrentMap["hash1"]; !ok {
		...
		if torrent.Name != "Torrent 1" {
			t.Errorf("hash1 name: expected %q, got %q", "Torrent 1", torrent.Name)
		}
		if torrent.State != "downloading" {
			t.Errorf("hash1 state: expected %q, got %q", "downloading", torrent.State)
		}
	}
	...
	if torrent, ok := torrentMap["hash2"]; !ok {
		...
		if torrent.Name != "Torrent 2" {
			t.Errorf("hash2 name: expected %q, got %q", "Torrent 2", torrent.Name)
		}
	}
...
func TestState_ApplyDeltaUpdate(t *testing.T) {
	...
	initialDelta := &API.DeltaMainData{
		...
		Tags:        []string{"tag1"},
		ServerState: API.ServerState{DHTNodes: 500},
	}
	...
	deltaDelta := &API.DeltaMainData{
		...
		ServerState: API.ServerState{DHTNodes: 600},
	}
	...
	if torrent, ok := torrentMap["hash1"]; !ok {
		...
		if torrent.Name != "Torrent 1" {
			t.Errorf("hash1 name should be preserved: expected %q, got %q", "Torrent 1", torrent.Name)
		}
		if torrent.State != "seeding" {
			t.Errorf("hash1 state should be updated: expected %q, got %q", "seeding", torrent.State)
		}
		if torrent.Dlspeed != 0 {
			t.Errorf("hash1 dlspeed should be updated: expected 0, got %d", torrent.Dlspeed)
		}
		if torrent.Progress != 1.0 {
			t.Errorf("hash1 progress should be updated: expected 1.0, got %f", torrent.Progress)
		}
	}
